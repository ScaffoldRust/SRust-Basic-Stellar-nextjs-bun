          ...
          <div className="col-span-4">
            ...
            <Card>
              <CardContent className="p-4">
                {(mode === 'read' ? readMethods : writeMethods).map((method, index) => (
                  <div key={index} className="pb-4">
                    <h3 className="text-lg font-semibold text-gray-900 mb-3">{method.name}</h3>
                    <div className="space-y-3">
                      {method.params.map((param, paramIndex) => (
                        <div key={paramIndex}>
                          <label className="block text-sm text-gray-600 mb-1">{param}</label>
                          <Input placeholder={`Enter ${param}`} />
                        </div>
                      ))}
                    </div>
                    <div className="flex justify-end mt-3">
                      <Button variant="default">{mode === 'read' ? 'Read' : 'Execute'}</Button>
                    </div>
                    {index !== (mode === 'read' ? readMethods : writeMethods).length - 1 && (
